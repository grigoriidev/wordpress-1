...
foreach($posts as $post){
	...
	if($img){
	 ...
	 $out .= '<center><a class="post-thumb" style="border-radius: 10px; clear:both;" href="' . $permalink . ' ">'
	...
	if($full || $readmore){
		...
		$out .= '<a style="font-size: 20px; text-decoration: none;" href="' . $permalink . '"><em>' . $title . ' &raquo; </em></a>';
		$out .= '<div class="post-content" style="clear:both";><!-- '.$content.'-->';
		
		//formatting for read more
		if($readmore){
			$out .= '<a href="' . $permalink . '" class="readmore" style="text-decoration: underline;">' . $readmore . '</a>';
		}
	...
	//If NOT full, output either the <li> or <h#> tags & the title of the post
		...
		$out .= '<a style="font-size: 25px;" href="' . $permalink . '"><em>' . $title . ' &raquo; </em></a>';	
