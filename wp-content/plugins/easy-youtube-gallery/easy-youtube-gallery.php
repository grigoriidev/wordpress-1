...
if ( ! class_exists( 'WPAU_EASY_YOUTUBE_GALLERY' ) ) {
	class WPAU_EASY_YOUTUBE_GALLERY
	{
		...
		function enqueue_scripts() {
			...
			wp_register_style(
				'easy-youtube-gallery',
				plugins_url( 'assets/css/eytg.min.css', __FILE__ ),
				array(),
				self::VER
			);
		...
		public function shortcode($atts) {
			...
			if ( empty( $id ) ) {
				return '<p class="eytg-error">' . __( 'Please provide ID’s for some YouTube videos', 'easy-youtube-gallery' ) . '</p>';
			}

			// Cleanup privacy and playsinline parameters
			$enhanceprivacy = ( empty( $privacy ) ) ? 0 : 1;
			$playsinline = ( empty( $playsinline ) ) ? 0 : 1;
			...
			foreach ( $ids as $video_id ) {
				...
				$output .= "<a href=\"//www.youtube.com/watch?v={$video_id}&amp;rel=0&amp;modestbranding=1&amp;controls={$controls}&amp;playsinline={$playsinline}&amp;enhanceprivacy={$enhanceprivacy}\" class=\"eytg-item eytg-item-{$item_num} eytg-item-{$item_position}\">";
